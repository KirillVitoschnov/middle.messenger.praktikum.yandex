...
describe('EventBus', () => {
    it('должен вызывать зарегистрированный обработчик', () => {
        ...
        bus.on('test', (value) => { result = value; });
        bus.emit('test', 42);
    ...
    it('не должен вызывать обработчик после удаления', () => {
        let result = 0;
        const bus = new EventBus<{ test: [number] }>();
        const handler = (value: number) => { result = value; };
        bus.on('test', handler);
    ...
    it('должен корректно обрабатывать несколько обработчиков', () => {
        ...
        let result2 = 0;
        const bus = new EventBus<{ test: [number] }>();
        bus.on('test', (value) => { result1 = value; });
        bus.on('test', (value) => { result2 = value; });
        bus.emit('test', 7);
    ...
    it('должен ловить ошибки в обработчике и продолжать выполнение', () => {
        let result = 0;
        const bus = new EventBus<{ test: [number] }>();
        bus.on('test', () => { throw new Error('Ошибка'); });
        bus.on('test', (value) => { result = value; });
        const errorStub: any[] = [];
        const originalConsoleError = console.error;
        console.error = () => { errorStub.push(true); };
        bus.emit('test', 99);
