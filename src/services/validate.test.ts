...
describe('Тестирование функций валидации', () => {
    ...
    it('Должен возвращать ошибку для обязательного поля, если оно пустое', () => {
        ...
        assert.equal(errorSpan?.textContent, 'Обязательное поле', 'Текст ошибки должен соответствовать');
    });
    ...
    it('Должен проверять правило минимальной длины', () => {
        ...
        assert.equal(errorSpan?.textContent, 'Минимальное число символов: 5', 'Текст ошибки должен содержать минимальную длину');
    });
    ...
    it('Должен валидировать форму с несколькими полями', () => {
        ...
        assert.isFalse(result, 'Форма должна быть невалидной, если хотя бы одно поле не прошло проверку');
